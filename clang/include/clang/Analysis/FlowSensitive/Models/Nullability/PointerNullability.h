...
#ifndef LLVM_CLANG_ANALYSIS_FLOWSENSITIVE_MODELS_NULLABILITY_POINTERNULLABILITY_H
#define LLVM_CLANG_ANALYSIS_FLOWSENSITIVE_MODELS_NULLABILITY_POINTERNULLABILITY_H
...
dataflow::PointerValue *getRawPointerValue(const Expr *PointerExpr,
...
dataflow::PointerValue *getPointerValueFromSmartPointer(
    dataflow::RecordStorageLocation *SmartPointerLoc,
    const dataflow::Environment &Env);
...
dataflow::PointerValue *getSmartPointerValue(const Expr *SmartPointerExpr,
                                             const dataflow::Environment &Env);
...
/// and may be null.
bool isNullable(PointerNullState NullState, const dataflow::Environment &Env,
...
getNullability(PointerNullState PointerNullState,
               const dataflow::Environment &Env,
               const dataflow::Formula *AdditionalConstraints = nullptr);

/// Returns the strongest provable assertion we can make about `PointerVal`.
clang::NullabilityKind
getNullability(const dataflow::PointerValue &PointerVal,
               const dataflow::Environment &Env,
               const dataflow::Formula *AdditionalConstraints = nullptr);
...
/// `E` in `Env`.
clang::NullabilityKind
getNullability(const Expr *E, const dataflow::Environment &Env,
               const dataflow::Formula *AdditionalConstraints = nullptr);
...
template <typename T> void dump(const T &Node, llvm::raw_ostream &OS) {
  clang::ASTDumper(OS, /*ShowColors=*/false).Visit(Node);
}

} // namespace clang::dataflow::nullability

#endif // LLVM_CLANG_ANALYSIS_FLOWSENSITIVE_MODELS_NULLABILITY_POINTERNULLABILITY_H
