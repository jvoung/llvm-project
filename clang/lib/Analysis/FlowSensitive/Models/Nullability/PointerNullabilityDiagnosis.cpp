...
#include "clang/Analysis/FlowSensitive/Models/Nullability/ForwardingFunctions.h"
...
untrackedError(const Expr *E, const ASTContext &Ctx,
...
static const Expr *
matchesNonConstCallNullCheck(const Expr &Pointer, ASTContext &Ctx,
  ...
  if (NonConstMethodAsPointerSrc == nullptr)
    return nullptr;
...
static bool isCapturedVariableOrMemberAccess(const Expr *E) {
...
static SmallVector<PointerNullabilityDiagnostic>
diagnoseNonnullExpected(const Expr *E, const Environment &Env, ASTContext &Ctx,
                        PointerNullabilityDiagnostic::Context DiagCtx,
...
static SmallVector<PointerNullabilityDiagnostic> diagnoseAssignmentLike(
    ...
    const Expr *RHS, const DiagTransferState &State, ASTContext &Ctx,
    PointerNullabilityDiagnostic::Context DiagCtx,
    const clang::NamedDecl *Callee = nullptr,
    const clang::IdentifierInfo *ParamName = nullptr,
...
static SmallVector<PointerNullabilityDiagnostic>
diagnoseDereference(const UnaryOperator *UnaryOp,
...
static SmallVector<PointerNullabilityDiagnostic>
diagnoseSmartPointerDereference(const CXXOperatorCallExpr *Op,
                                const MatchFinder::MatchResult &Result,
                                const DiagTransferState &State) {
  return diagnoseNonnullExpected(
...
static SmallVector<PointerNullabilityDiagnostic>
diagnoseSubscript(const ArraySubscriptExpr *Subscript,
                  const MatchFinder::MatchResult &Result,
                  const DiagTransferState &State) {
  return diagnoseNonnullExpected(
...
static SmallVector<PointerNullabilityDiagnostic>
diagnoseArrow(const MemberExpr *MemberExpr,
              const MatchFinder::MatchResult &Result,
              const DiagTransferState &State) {
  return diagnoseNonnullExpected(
...
static SmallVector<PointerNullabilityDiagnostic>
diagnoseAssignment(const BinaryOperator *Op,
                   const MatchFinder::MatchResult &Result,
                   const DiagTransferState &State) {
...
static SmallVector<PointerNullabilityDiagnostic>
diagnoseSmartPointerAssignment(const CXXOperatorCallExpr *Op,
                               const MatchFinder::MatchResult &Result,
                               const DiagTransferState &State) {
  const TypeNullability *LHSNullability =
...
static SmallVector<PointerNullabilityDiagnostic> diagnoseArgumentCompatibility(
    ...
    const clang::NamedDecl *Callee, const DiagTransferState &State,
    ASTContext &Ctx) {
...
/// \endcode
static SmallVector<PointerNullabilityDiagnostic>
diagnoseAssertNullabilityCall(const CallExpr *CE,
  ...
  if (MaybeComputed == nullptr)
    return untrackedError(CE, Ctx);

  if (*MaybeComputed == Expected)
    return {};
...
static SmallVector<PointerNullabilityDiagnostic>
diagnoseIncrementDecrement(const UnaryOperator *UnaryOp,
                           const MatchFinder::MatchResult &Result,
                           const DiagTransferState &State) {
...
static SmallVector<PointerNullabilityDiagnostic>
diagnoseAddSubtract(Expr *PtrExpr, Expr *IntExpr, const Environment &Env,
                    ASTContext &Ctx) {
...
static SmallVector<PointerNullabilityDiagnostic>
diagnoseAddSubtractAssign(const BinaryOperator *BinaryOp,
                          const MatchFinder::MatchResult &Result,
                          const DiagTransferState &State) {
...
static SmallVector<PointerNullabilityDiagnostic>
diagnoseAddSubtractInteger(const BinaryOperator *BinaryOp,
                           const MatchFinder::MatchResult &Result,
                           const DiagTransferState &State) {
...
static SmallVector<PointerNullabilityDiagnostic>
diagnosePointerDifference(const BinaryOperator *BinaryOp,
                          const MatchFinder::MatchResult &Result,
                          const DiagTransferState &State) {
  SmallVector<PointerNullabilityDiagnostic> Diagnostics =
...
static SmallVector<PointerNullabilityDiagnostic>
diagnoseConstructExpr(const CXXConstructExpr *CE,
                      const MatchFinder::MatchResult &Result,
                      const DiagTransferState &State) {
...
static SmallVector<PointerNullabilityDiagnostic>
diagnoseMakeUniqueConstructExpr(const CallExpr *MakeUniqueCall,
                                const CXXConstructExpr *CEInMakeUnique,
                                const MatchFinder::MatchResult &Result,
                                const DiagTransferState &State) {
...
diagnoseMakeUniqueParenInitListExpr(
    ...
    const CXXParenListInitExpr *InitListInMakeUnique,
    const MatchFinder::MatchResult &Result, const DiagTransferState &State) {
  if (!InitListInMakeUnique->getType()->isRecordType())
    return {};
...
static SmallVector<PointerNullabilityDiagnostic>
diagnoseCallExpr(const CallExpr *CE, const MatchFinder::MatchResult &Result,
                 const DiagTransferState &State) {
...
static SmallVector<PointerNullabilityDiagnostic>
diagnoseReturn(const ReturnStmt *RS, const MatchFinder::MatchResult &Result,
               const DiagTransferState &State) {
  if (!RS->getRetValue())
    return {};
...
static SmallVector<PointerNullabilityDiagnostic>
diagnoseMemberInitializer(const CXXCtorInitializer *CI,
                          const MatchFinder::MatchResult &Result,
                          const DiagTransferState &State) {
...
static SmallVector<PointerNullabilityDiagnostic>
diagnoseInitListExpr(const InitListExpr *ILE,
                     const MatchFinder::MatchResult &Result,
                     const DiagTransferState &State) {
...
static SmallVector<PointerNullabilityDiagnostic>
diagnoseDeclStmt(const DeclStmt *DS, const MatchFinder::MatchResult &Result,
                 const DiagTransferState &State) {
  SmallVector<PointerNullabilityDiagnostic> Diagnostics;
...
static SmallVector<PointerNullabilityDiagnostic>
diagnoseMovedFromNonnullSmartPointer(const Expr *E,
                                     const MatchFinder::MatchResult &Result,
                                     const DiagTransferState &State) {
  const TypeNullability *Nullability = State.Lattice.getTypeNullability(E);
  const auto &Ctx = *Result.Context;
  if (Nullability == nullptr)
    return untrackedError(E, Ctx);

  if (Nullability->front().concrete() != NullabilityKind::NonNull)
    return {};
...
static void checkAnnotationsConsistent(
    const ValueDecl *VD, llvm::SmallVector<PointerNullabilityDiagnostic> &Diags,
    const TypeNullabilityDefaults &Defaults) {
...
static DiagTransferFunc pointerNullabilityDiagnoserAfter(
  ...
  return CFGMatchSwitchBuilder<const DiagTransferState,
      ...
      .CaseOfCFGStmt<Expr>(
          ...
          [&AllowedMovedFromNonnull](const Expr *E,
                                     const MatchFinder::MatchResult &Result,
                                     const DiagTransferState &State)
              -> SmallVector<PointerNullabilityDiagnostic> {
            if (AllowedMovedFromNonnull.allowed(E))
              return {};
