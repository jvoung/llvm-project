...
const Expr *findMakeUniqueNewExprInitializer(const Stmt *S) {
  ...
  for (const Stmt *Child : S->children()) {
    if (auto *E = findMakeUniqueNewExprInitializer(Child))
      return E;
  }
...
const Expr *getUnderlyingInitExprInStdMakeUnique(const FunctionDecl &Decl) {
  if (!isStdMakeUniqueWithNontrivalConstructor(Decl))
    return nullptr;
