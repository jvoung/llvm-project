...
PointerValue *getRawPointerValue(const Expr *PointerExpr,
                                 const Environment &Env) {
  return Env.get<PointerValue>(*PointerExpr);
}

PointerValue *
getPointerValueFromSmartPointer(RecordStorageLocation *SmartPointerLoc,
                                const Environment &Env) {
...
PointerValue *getSmartPointerValue(const Expr *SmartPointerExpr,
                                   const Environment &Env) {
...
dataflow::PointerValue *getPointerValue(const Expr *PointerExpr,
                                        const Environment &Env) {
...
void setSmartPointerValue(dataflow::RecordStorageLocation &SmartPointerLoc,
                          dataflow::PointerValue *Val, Environment &Env) {
...
static bool tryCreatePointerNullState(PointerValue &PointerVal,
  ...
  if (hasPointerNullState(PointerVal))
    return false;
  if (!FromNullable)
    FromNullable = &A.makeAtomRef(A.makeAtom());
  if (!IsNull)
...
bool isNullable(PointerNullState PointerNullState, const Environment &Env,
  ...
  if (Null) {
    if (!AdditionalConstraints && Null->isLiteral(false))
      return false;
    ...
    if (Env.proves(*ProvablyNull)) {
      ...
      if (Env.proves(A.makeLiteral(false)))
        return false;
      return true;
    }
  }
  ...
  if (Null)
    NullableAndMaybeNull = &A.makeAnd(*NullableAndMaybeNull, *Null);
  if (AdditionalConstraints)
...
bool isNullable(const PointerValue &PointerVal, const Environment &Env,
                const dataflow::Formula *AdditionalConstraints) {
  return isNullable(getPointerNullState(PointerVal), Env,
                    AdditionalConstraints);
}

bool isReachableNullptrLiteral(const Environment &Env,
                               const dataflow::Formula *AdditionalConstraints) {
  auto &A = Env.getDataflowAnalysisContext().arena();
  PointerNullState NullState = {/*FromNullable=*/&A.makeLiteral(true),
                                /*IsNull=*/&A.makeLiteral(true)};
  return isNullable(NullState, Env, AdditionalConstraints);
}

NullabilityKind getNullability(PointerNullState PointerNullState,
                               const dataflow::Environment &Env,
                               const dataflow::Formula *AdditionalConstraints) {
  auto &A = Env.getDataflowAnalysisContext().arena();
  if (auto *Null = PointerNullState.IsNull) {
    if (AdditionalConstraints)
      Null = &A.makeAnd(*AdditionalConstraints, *Null);
    if (Env.proves(A.makeNot(*Null)))
      return NullabilityKind::NonNull;
  }
  return isNullable(PointerNullState, Env, AdditionalConstraints)
...
NullabilityKind getNullability(const dataflow::PointerValue &PointerVal,
                               const dataflow::Environment &Env,
                               const dataflow::Formula *AdditionalConstraints) {
  return getNullability(getPointerNullState(PointerVal), Env,
                        AdditionalConstraints);
}
...
NullabilityKind
getNullabilityForNullptrT(const dataflow::Environment &Env,
                          const dataflow::Formula *AdditionalConstraints) {
