...
PointerNullabilityAnalysis::assignNullabilityVariable(const ValueDecl *D,
                                                      dataflow::Arena &A) {
  auto [It, Inserted] = NFS.DeclTopLevelNullability.try_emplace(
      cast<ValueDecl>(D->getCanonicalDecl()));
  if (Inserted)
    It->second = PointerTypeNullability::createSymbolic(A);
  return It->second;
}
...
static const Formula *mergeFormulas(const Formula *Bool1,
  ...
  if (Bool1 == nullptr || Bool2 == nullptr)
    return nullptr;
...
ComparisonResult PointerNullabilityAnalysis::compare(QualType Type,
  ...
  if (const auto *PointerVal1 = dyn_cast<PointerValue>(&Val1)) {
    ...
    if (!hasPointerNullState(*PointerVal1))
...
// returns `Top`.
static std::pair<const Formula *, LatticeEffect>
widenNullabilityProperty(const Formula *Prev, const Environment &PrevEnv,
                         const Formula *Cur, Environment &CurEnv) {
  if (Prev == Cur)
    return {Cur, LatticeEffect::Unchanged};
  if (Prev == nullptr)
    return {nullptr, LatticeEffect::Unchanged};
  if (Cur == nullptr)
...
PointerNullabilityAnalysis::widen(QualType Type, Value &Prev,
...
StorageLocation &PointerNullabilityAnalysis::getTopStorageLocation(
  ...
  if (Inserted)
    It->second = &DACtx.createStorageLocation(Ty);
  return *It->second;
}
