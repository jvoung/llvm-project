...
#include "clang/Analysis/FlowSensitive/Models/Nullability/TypeAndMaybeLocVisitor.h"
...
getSmartPointerBaseClass(const CXXRecordDecl *RD,
      ...
      if (Base.getAccessSpecifier() <= BaseAccess) {
        ...
        if (BaseClass == nullptr) {
          if (const auto *TST =
            ...
            // a specialization.)
            if (BaseClass != nullptr) {
              if (!Seen.insert(BaseClass).second)
                return nullptr;
            }
          }
        }
...
static const CXXRecordDecl *
getSmartPointerBaseClass(const CXXRecordDecl *RD, AccessSpecifier BaseAccess) {
  llvm::SmallPtrSet<const CXXRecordDecl *, 2> Seen;
...
PointerTypeNullability::createSymbolic(dataflow::Arena &A) {
...
PointerTypeNullability
PointerTypeNullability::createSymbolic(dataflow::Atom Nonnull,
...
FileID getGoverningFile(const Decl *D) {
  if (!D)
    return FileID();
...
std::optional<NullabilityKind> getAliasNullability(const TemplateName &TN) {
  if (const auto *TD = TN.getAsTemplateDecl()) {
    if (!TD->getTemplatedDecl())
      return std::nullopt; // BuiltinTemplateDecl
    if (const auto *A = TD->getTemplatedDecl()->getAttr<AnnotateAttr>()) {
      if (A->getAnnotation() == "Nullable")
        return NullabilityKind::Nullable;
      if (A->getAnnotation() == "Nonnull")
...
std::optional<QualType> unwrapAlias(QualType T) {
  ...
  while (true) {
    ...
    if (Next.hasLocalQualifiers())
      return std::nullopt;
    if (Next.getTypePtr() == T.getTypePtr())
      return std::nullopt; // not sugar
...
private:
  ...
  std::vector<TemplateContext> getBoundTemplateArgsFromQualifiedType(
      const Type *T, std::optional<NestedNameSpecifierLoc> NNSLoc) {
    ...
    for (NestedNameSpecifier NNS = T->getPrefix(); NNS;) {
      ...
      case NestedNameSpecifier::Kind::MicrosoftSuper:
        NNS = std::nullopt;
        if (NNSLoc)
          NNSLoc = clang::NestedNameSpecifierLoc();
        break;
      ...
      case NestedNameSpecifier::Kind::Type:
        ...
        if (NNSLoc)
          NNSLoc = NNSLoc->getAsTypeLoc().getPrefix();
        break;
      default:
        NNS = std::nullopt;
        if (NNSLoc)
  ...
  getBoundTemplateArgsFromQualifiedType(const Type *T,
                                        std::optional<TypeLoc> L) {
...
public:
  ...
  void visit(const TemplateArgument &TA,
    ...
    case TemplateArgument::Type: {
      ...
      if (ArgLoc)
        recordLoc(*ArgLoc, /*OverrideQualifiedType=*/true);
  ...
  void visit(const Type *T, std::optional<TypeLoc> L) {
    if (L)
      recordLoc(*L);
    Base::visit(T, L);
  }
  ...
  std::vector<TemplateContext>
  getBoundTemplateArgsFromOtherQualifiedTypes(const Type *T,
                                              std::optional<TypeLoc> L) {
    ...
    case Type::DependentName:
      return getBoundTemplateArgsFromQualifiedType<DependentNameTypeLoc>(T, L);
    case Type::Enum:
  ...
  void visitType(const Type *T, std::optional<TypeLoc> L) {
    std::vector<TemplateContext> BoundTemplateArgs =
  ...
  void visitTemplateSpecializationType(
    ...
    if (TST->isTypeAlias()) {
      ...
      if (NK == NullabilityKind::Unspecified) {
        ...
        if (!Inner || !isUnknownValidOn(*Inner))
          NK = std::nullopt;
      }
      if (NK)
        sawNullability(*NK);
  ...
  void visitSubstTemplateTypeParmType(
    ...
    for (const auto *Ctx = CurrentTemplateContext; Ctx; Ctx = Ctx->Extends) {
      if (T->getAssociatedDecl() != Ctx->AssociatedDecl)
        continue;
      // If args are not available, fall back to un-sugared arg.
      if (!Ctx->Args.has_value())
        break;
  ...
  void visitRecordType(const RecordType *RT, std::optional<RecordTypeLoc> L) {
    std::vector<TemplateContext> BoundTemplateArgs =
        getBoundTemplateArgsFromQualifiedType(
  ...
  void visitAttributedType(const AttributedType *AT,
    ...
    if (NK == NullabilityKind::Unspecified) {
      if (!isUnknownValidOn(AT->getModifiedType()))
        NK = std::nullopt;
    }
    if (NK)
      sawNullability(*NK);
  ...
  void visitArrayType(const ArrayType *AT, std::optional<ArrayTypeLoc> L) {
    ignoreUnexpectedNullability();
    visit(AT->getElementType(),
          L ? std::optional<TypeLoc>(L->getElementLoc()) : std::nullopt);
  }

  void visitParenType(const ParenType *PT, std::optional<ParenTypeLoc> L) {
    visit(PT->getInnerType(),
          L ? std::optional<TypeLoc>(L->getInnerLoc()) : std::nullopt);
  }

  void visitAutoType(const AutoType *AT, std::optional<AutoTypeLoc> L) {
...
struct CountWalker : public NullabilityWalker<CountWalker> {
  ...
  unsigned Count = 0;
  void report(const Type *, FileID, std::optional<NullabilityKind>,
              std::optional<TypeLoc>) {
    ++Count;
  }
};
...
TypeNullability getTypeNullability(
  ...
  struct Walker : NullabilityWalker<Walker> {
    ...
    void report(const Type *, FileID File, std::optional<NullabilityKind> NK,
                std::optional<TypeLoc>) {
      if (!NK)
        NK = Defaults.get(File);
...
std::vector<TypeNullabilityLoc>
getTypeNullabilityLocs(TypeLoc Loc, const TypeNullabilityDefaults &Defaults) {
  ...
  struct Walker : NullabilityWalker<Walker> {
    ...
    void report(const Type *T, FileID File, std::optional<NullabilityKind> NK,
