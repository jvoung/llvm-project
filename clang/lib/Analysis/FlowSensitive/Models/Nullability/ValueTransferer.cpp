...
#include "clang/Analysis/FlowSensitive/Models/Nullability/ValueTransferer.h"
...
#include "clang/Analysis/FlowSensitive/Models/Nullability/AstHelpers.h"
#include "clang/Analysis/FlowSensitive/Models/Nullability/MacroArgCapture.h"
#include "clang/Analysis/FlowSensitive/Models/Nullability/PointerNullability.h"
#include "clang/Analysis/FlowSensitive/Models/Nullability/PointerNullabilityLattice.h"
#include "clang/Analysis/FlowSensitive/Models/Nullability/PointerNullabilityMatchers.h"
...
PointerValue *ensureRawPointerHasValue(const Expr *E, Environment &Env) {
  if (!isSupportedRawPointerType(E->getType()))
    return nullptr;

  if (E->isPRValue()) {
    if (auto *Val = Env.get<PointerValue>(*E))
      return Val;
  ...
  if (auto *Val = Env.get<PointerValue>(*Loc))
    return Val;
  auto *Val = cast<PointerValue>(Env.createValue(E->getType()));
...
static PointerTypeNullability
getPointerTypeNullability(const Expr *E, PointerNullabilityLattice &L) {
...
static PointerValue *unpackPointerValue(StorageLocation &PointerLoc,
                                        Environment &Env) {
  ...
  if (NullState.FromNullable && NullState.IsNull)
    return PointerVal;
...
static void
initSmartPointerForExpr(const Expr *E,
                        TransferState<PointerNullabilityLattice> &State) {
...
static void
transferNullPointer(const Expr *NullPointer, const MatchFinder::MatchResult &,
                    TransferState<PointerNullabilityLattice> &State) {
...
static void
transferPointerIncOrDec(const UnaryOperator *UnaryOp,
...
static void
transferPointerAddOrSubAssign(const BinaryOperator *BinaryOp,
                              const MatchFinder::MatchResult &,
                              TransferState<PointerNullabilityLattice> &State) {
...
static void
transferNotNullPointer(const Expr *NotNullPointer,
                       const MatchFinder::MatchResult &,
                       TransferState<PointerNullabilityLattice> &State) {
...
static QualType
underlyingRawPointerTypeFromSmartPointer(RecordStorageLocation &Loc) {
...
static void
transferSmartPointerResetCall(const CXXMemberCallExpr *MCE,
...
static void
transferSmartPointerGetCall(const CXXMemberCallExpr *MCE,
                            const MatchFinder::MatchResult &Result,
                            TransferState<PointerNullabilityLattice> &State) {
  // If the return type isn't what we expect, bail out.
...
static QualType
getReceiverIgnoringImpCastsType(const CXXOperatorCallExpr *OpCall) {
...
static void transferSmartPointerFactoryCall(
  ...
  if (isSupportedRawPointerType(PtrLoc.getType()->getPointeeType()) &&
    ...
    if (!ArgPV || !hasPointerNullState(*ArgPV))
      return;
...
static void
transferWrapUniqueCall(const CallExpr *CE,
                       const MatchFinder::MatchResult &Result,
                       TransferState<PointerNullabilityLattice> &State) {
  if (CE->getNumArgs() != 1) {
    return;
...
static void transferSmartPointerComparisonOpCall(
  ...
  if (!NullPtr1)
    Val1 = getSmartPointerValue(OpCall->getArg(0), State.Env);

  PointerValue *Val2 = nullptr;
  if (!NullPtr2)
    Val2 = getSmartPointerValue(OpCall->getArg(1), State.Env);
...
static void
transferSharedPtrCastCall(const CallExpr *CE,
                          const MatchFinder::MatchResult &Result,
                          TransferState<PointerNullabilityLattice> &State) {
...
static void
transferWeakPtrLockCall(const CXXMemberCallExpr *MCE,
                        const MatchFinder::MatchResult &Result,
                        TransferState<PointerNullabilityLattice> &State) {
...
static void transferSmartPointerArrowMemberExpr(
  ...
  if (const auto *TyNullability =
    ...
    if (TyNullability->size() >= 2)
      Nullability = (*TyNullability)[1];
  }
...
static void transferPointer(const Expr *PointerExpr,
                            const MatchFinder::MatchResult &Result,
                            TransferState<PointerNullabilityLattice> &State) {
...
static BoolValue *processPointerComparison(const Formula &ComparisonFormula,
                                           const Formula *LHSNull,
                                           const Formula *RHSNull,
...
static void
transferNullCheckComparison(const BinaryOperator *BinaryOp,
                            const MatchFinder::MatchResult &Result,
                            TransferState<PointerNullabilityLattice> &State) {
  ...
  if (!LHSVal || !hasPointerNullState(*LHSVal))
    return;
  auto *RHSVal = getRawPointerValue(RHS, State.Env);
  if (!RHSVal || !hasPointerNullState(*RHSVal))
    return;
...
static void
initializeOutputParameter(const Expr *Arg,
                          TransferState<PointerNullabilityLattice> &State,
                          const VarDecl &Param) {
  ...
  if (!ParamTy->isPointerType() && !ParamTy->isReferenceType())
    return;
  if (!isSupportedPointerType(ParamTy->getPointeeType()))
    return;
...
static void modelCheckNE(const CallExpr &CE, Environment &Env) {
  ...
  if (!isSupportedPointerType(LTy) && !LTy->isNullPtrType())
    return;
  if (!isSupportedPointerType(RTy) && !RTy->isNullPtrType())
    return;
...
static void transferCallExpr(const CallExpr *CE,
                             const MatchFinder::MatchResult &Result,
                             TransferState<PointerNullabilityLattice> &State) {
  ...
  if (FuncDecl != nullptr) {
    if ((FunII = FuncDecl->getDeclName().getAsIdentifierInfo())) {
      if (FunII->isStr("__assert_nullability"))
        return;
  ...
  if (CE->isCallToStdMove() || FuncDecl == nullptr)
    return;
...
static void
transferAccessorCall(const CXXMemberCallExpr *MCE,
                     const MatchFinder::MatchResult &Result,
                     TransferState<PointerNullabilityLattice> &State) {
...
initCallbackForStorageLocationIfSmartPointer(const CallExpr *CE,
...
static void
handleConstMemberCall(const CallExpr *CE,
...
static void
transferConstMemberCall(const CXXMemberCallExpr *MCE,
                        const MatchFinder::MatchResult &Result,
                        TransferState<PointerNullabilityLattice> &State) {
...
static void
handleNonConstMemberCall(const CallExpr *CE,
                         dataflow::RecordStorageLocation *RecordLoc,
                         const MatchFinder::MatchResult &Result,
                         TransferState<PointerNullabilityLattice> &State) {
  ...
  if (RecordLoc != nullptr) {
    for (const auto [Field, FieldLoc] : RecordLoc->children()) {
      ...
      if (isa<ClassTemplateSpecializationDecl>(
        ...
        // clear the value.
        // TODO: We should resugar the type of the field, similar to
...
static void
transferNonConstMemberCall(const CXXMemberCallExpr *MCE,
                           const MatchFinder::MatchResult &Result,
                           TransferState<PointerNullabilityLattice> &State) {
...
static void
transferStatusOrValueOrCall(const CXXMemberCallExpr *MCE,
                            const MatchFinder::MatchResult &Result,
                            TransferState<PointerNullabilityLattice> &State) {
  ...
  if (ResultState.FromNullable != nullptr) {
    if (ArgState->FromNullable == nullptr) {
      ...
      DACtx.addInvariant(
          A.makeImplies(StatusOrIsOk, A.makeEquals(*ResultState.FromNullable,
                                                   OldResultFromNullable)));
...
buildValueTransferer() {
  ...
  return dataflow::CFGMatchSwitchBuilder<
      ...
      // TODO: I believe we should be able to move handling of null
...
void ensureSmartPointerInitialized(
  ...
  if (E == nullptr || !isSupportedSmartPointerType(E->getType()))
    return;
